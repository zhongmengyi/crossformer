    ...
    def test(self, setting, save_pred = False, inverse = False):
        ...
        with torch.no_grad():
            for i, (batch_x,batch_y) in enumerate(test_loader):
                ...
                if (save_pred):
                    mean, std = self.Data1.scaler.mean, self.Data1.scaler.std  # 2023/6/1
                    preds.append(pred.detach().cpu().numpy()*std+mean)         #
                    trues.append(true.detach().cpu().numpy()*std+mean)         #
